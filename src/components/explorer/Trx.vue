  ...
  <div class="trx">
    ...
    ><span class="operation break-word">
      ...
      <div v-else-if="typeOp == 'claim_reward_balance'">
        <router-link :to="EXPLORER+'@'+op.account">{{op.account}}</router-link> claim reward: {{this.vests2sp(op.reward_vests)}} - {{op.time}}
      </div>
      ...
      <div v-else-if="typeOp == 'author_reward'">
        <router-link :to="EXPLORER+'@'+op.author">{{op.author}}</router-link> author reward: {{this.vests2sp(op.vesting_payout)}} for <router-link :to="EXPLORER+link(op.author,op.permlink)">{{linkCut(op.author,op.permlink)}}</router-link>
         - {{op.time}}
      </div>
